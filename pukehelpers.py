...
######## Force root + confirm verification
# Helper to enforce: being on linux, being root, the user agrees
def adultverification(message):
    if System.OS == System.LINUX:
        console.warn(message)
        if System.LOGIN != 'root':
            console.fail('You need to be ROOT')
        resp = prompt('ARE YOU OVER 18? (y/[N])', 'N')
        if resp != 'y':
            console.fail('Don\'t go all whinny now, hey? You will grow up at some point. Maybe. Later. And if you don\'t, well... You can still grow cows ya know?')
